...
It does not own detailed server process decomposition, deployment, storage, or recovery policy. Read `multiplayer-server-architecture.md` for server architecture. Read `multiplayer-protocol.md` for protocol design, serialization, heartbeat, and reconnect rules.
...
- Read `multiplayer-server-architecture.md` for process roles, deployment profile, ownership, persistence, and framework family guidance.
