...
- Test-only APIs or members—named `ForTest`, `TestOnly`, `__test`, `ExposeInternal`, `ResetForTest`, `GetInternalState`, or similar—including test-only getters, setters, delegates, proxies, callbacks, or hooks. See Testing Rules for the full backdoor taxonomy.
...
- Do not add a port just for testability. If a unit test can't be written without faking a type that has no real variant, that test belongs to integration (see Testing Rules), not the unit suite.
...
Rules that span both languages live in their canonical sections: do not return internal mutable collections (Domain Model and Encapsulation), do not bypass private access for tests (Testing Rules), and keep the View wrapper a plain class with a private native node (Domain-Controlled View Wrapper).

## 3. Testing Rules

The public surface must not grow test-only members; that API-side principle lives in Public API Gate. This part covers the testing side: replaceable seams, black-box behavior, and the unit/integration split.
