...
在 C# 和 TypeScript 游戏项目中，以领域封装和可验证的玩法行为为核心完成开发。不要为了局部实现或测试便利扩大公共表面、暴露运行时状态、让 Domain 耦合引擎对象或绕过分层。
...
- 仅供断言使用的中间状态；
- `ForTest`、`TestOnly`、`__test`、`ExposeInternal`、`ResetForTest`、`GetInternalState` 等 API；
- 测试专用 getter、setter、delegate、proxy、callback 或 hook。
...
- 不要仅为可测性而新增 port。若一个没有真实变种的类型无法在不 fake 的情况下写单元测试，该测试应归入集成（见行为测试规则），而不是单元测试。
...
- 不用类型断言或 bracket notation 绕过封装。

### C#

- 领域状态使用 private field 和受控的 `private set`；
- 优先让普通 C# Domain Object 持有与引擎无关的 View 契约；让具体 View wrapper 保持普通类，并用 private field 持有原生节点，而不是继承 `MonoBehaviour` 或 `Godot.Node`；
- 引擎支持时使用 `[SerializeField] private` 完成编辑器绑定；不要仅为 Inspector 访问把玩法状态设为 public；
- 不返回内部 `List<T>`、`Dictionary<TKey,TValue>` 等可变集合；
- 需要查询时返回不可变值、只读 DTO 或防御性 snapshot；
- 不为测试添加 `InternalsVisibleTo`，不反射读写私有状态。
